...
public class IPValidaton {
    ...
    public static boolean checkLoopBackIP(String ip) {
        String LOOPBACK_ADDRESS_REGEXP = "^localhost$|^127(?:\\.[0-9]+){0,2}\\.[0-9]+$|^(?:0*\\:)*?:?0*1$";
        final Pattern loopbackAddressPattern = Pattern.compile(LOOPBACK_ADDRESS_REGEXP);
        return loopbackAddressPattern.matcher(ip).matches();
    }
}
