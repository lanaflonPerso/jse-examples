...
public class CountInversions {
    public static long countInversions(int[] array) {
        return countInversions(array, 0, array.length - 1, new int[array.length]);
    }

    // Recursively counts the inversions.
    private static long countInversions(int[] array, int start, int end, int[] merged) {
        if (start == end) {
        ...
        return leftInversions + rightInversions + mergeAndCountInversions(left, right, merged);
    }
    ...
    private static long mergeAndCountInversions(int[] foo, int[] bar, int[] merged) {
        if (foo == null || bar == null || merged == null) {
            throw new NullPointerException("One or more of foo/bar/merged is null.");
        }
