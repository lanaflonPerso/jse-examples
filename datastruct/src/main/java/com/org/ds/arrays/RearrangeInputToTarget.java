...
/**
 * You are given two integer com.org.ds.arrays: input and target. These com.org.ds.arrays contain the same unique
 * integers, and always the value zero in each of them. Also, the target array is some permutation of the input array
 * and both com.org.ds.arrays are of the same size.
 *
 * You are supposed to write a method to convert the input array to the target array. The restriction is that you can
 * modify the input array by repeating only the following operation any number of times: Take any value in the input
...
public class RearrangeInputToTarget {
    ...
    public static void rearrange(int[] input, int[] target) {
        ...
        int zeroIndex = com.org.ds.common.ArrayUtils.findIndex(input, 0);
        int numMoves = 0;
        ...
        while (numMoves < input.length) {
            ...
            if (target[zeroIndex] == 0) {
                ...
                com.org.ds.common.ArrayUtils.swap(input, zeroIndex, outOfOrderIndex);
                ...
                com.org.ds.common.ArrayUtils.swap(input, zeroIndex, nextZeroIndex);
