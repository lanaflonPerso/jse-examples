 ...
 * The API should enable the clients (people in the city) to do the following:
 * 
 * When a client requests for a phone number, it allots one of the available numbers to them. Some clients may want
 * fancy numbers, so they can specifically ask for a number to be alloted to them. If the requested number is available
 * then the system allots it to them, otherwise the system allots any available number.
 * 
 * The system need not have to know which number is alloted to which client. The same client may make successive
 * requests and get multiple phone numbers for himself, but the system is not bothered. At any point of time, the system
 * only knows which phone numbers are alloted and which phone numbers are free.
 * 
 * To be very specific, the phone number allotter class should implement the PhoneNumberAllotter interface below.
 */
...
interface PhoneNumberAllotter {
    ...
    /**
     * Allot the requested fancy number. If the fancy number is taken, then allot any available number. Return the
     * allotted number.
     * 
     * @throws NoNumbersLeftException
     *             if no phone numbers are available to be allotted.
...
/**
 * Stores allotted and unallotted phone numbers in an array. Implementation of Get operations works with constant time
 * complexity.
 */
public class SimplePhoneNumberAllotter implements PhoneNumberAllotter {
    ...
    public SimplePhoneNumberAllotter(int size) {
        if (size < 0) {
            throw new IllegalArgumentException(String.format("Size cannot be negative. You passed: %d", size));
        }
        this.size = size;
    ...
    public int get(int fancyNumber) {
        if (fancyNumber >= phoneNumbers.length) {
            throw new IllegalArgumentException(
                    String.format("Phone numbers requested should be in the range: %d to %d. You passed: %d.", 0,
                            size - 1, fancyNumber));
        }
