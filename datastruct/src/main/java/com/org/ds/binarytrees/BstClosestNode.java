...
/**
 * Given the root of a binary search tree and a node from the same tree, find the node closest to the given node.
 * 
...
public class BstClosestNode {
    public static TreeNode<Integer> findClosestNode(TreeNode<Integer> bstRootNode, TreeNode<Integer> node) {
        TreeNode<Integer> inorderPredecessor = getInorderPredecessor(bstRootNode, node);
        TreeNode<Integer> inorderSuccessor = getInorderSuccessor(bstRootNode, node);
        ...
        if ((node.getValue() - inorderPredecessor.getValue()) < (inorderSuccessor.getValue() - node.getValue())) {
            return inorderPredecessor;
        }
    ...
    public static TreeNode<Integer> getInorderSuccessor(TreeNode<Integer> bstRootNode, TreeNode<Integer> node) {
    ...
    public static TreeNode<Integer> getInorderPredecessor(TreeNode<Integer> bstRootNode, TreeNode<Integer> node) {
        if (bstRootNode == null || node == null) {
            return null;
        }
