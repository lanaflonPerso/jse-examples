...
public class BstFindMode {
    ...
    private static <T> void findMode(TreeNode<T> root, Pair<T, Integer> mode) {
        ...
        if (current.getFirst() != null && (mode.getFirst() == null || current.getSecond() > mode.getSecond())) {
            mode.setFirst(current.getFirst());
            mode.setSecond(current.getSecond());
        }
    }

    private static <T> void findMode(TreeNode<T> root, Pair<T, Integer> mode, Pair<T, Integer> current) {
        if (root == null) {
            return;
        }
        ...
        if (current.getFirst() == null) {
            ...
            if (current.getFirst().equals(root.getValue())) {
                ...
                if (mode.getFirst() == null || current.getSecond() > mode.getSecond()) {
                    mode.setFirst(current.getFirst());
                    mode.setSecond(current.getSecond());
                }
