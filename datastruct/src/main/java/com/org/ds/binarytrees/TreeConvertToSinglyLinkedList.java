...
/**
 * Given a binary tree, convert it into a singly linked list. The "next" pointer in each node of the list is defined
 * using the "right" pointer in the binary tree. The order of nodes in the linked list is the same order that occurs in
 * the inorder representation of the binary tree. Also the solution must set all "left" pointers in each node to null.
 * 
 * Signature of expected method:
 * 
 * public static TreeNode<Integer> convertToSinglyLinkedList( TreeNode<Integer> root) {...}
 */
public class TreeConvertToSinglyLinkedList {
    ...
    private static Pair<TreeNode<Integer>, TreeNode<Integer>> doConnectInorder(TreeNode<Integer> root) {
        if (root == null) {
            return null;
        }

        Pair<TreeNode<Integer>, TreeNode<Integer>> toReturn = new Pair<TreeNode<Integer>, TreeNode<Integer>>();
        Pair<TreeNode<Integer>, TreeNode<Integer>> leftPair = doConnectInorder(root.getLeft());
        if (leftPair != null) {
        ...
        Pair<TreeNode<Integer>, TreeNode<Integer>> rightPair = doConnectInorder(root.getRight());
        if (rightPair != null) {
