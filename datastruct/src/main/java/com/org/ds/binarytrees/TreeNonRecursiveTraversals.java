...
/**
 * Given the root of a binary tree, print its pre-order, in-order and post-order traversal to standard out.
 * 
 * Signature of expected methods:
 * 
 * public static void traversePreorder(TreeNode<?> rootNode) {...} public static void traversePreorder(TreeNode
 * <?> rootNode) {...} public static void traversePreorder(TreeNode<?> rootNode) {...} public static void
 * traverseLevelorder(TreeNode<?> rootNode) {...}
 * 
 * Given the root of a binary tree, print the level order traversal for a specific level.
 * 
...
public class TreeNonRecursiveTraversals {
    ...
    public static void traversePostorder(TreeNode<?> rootNode) {
        ...
        while (!childStack.empty()) {
            ...
            if (!rootStack.empty() && current == rootStack.peek()) {
                ...
                // right sub-com.org.ds.trees of current.
                System.out.println(current);
                childStack.pop();
                rootStack.pop();
            } else {
    ...
    public static void traverseLevelorder(TreeNode<?> rootNode, int level) {
        if (level < 0) {
            throw new IllegalArgumentException(String.format("level should be >= 0. You passed: %d.", level));
        }
        ...
        if (levelCount != level) {
            throw new IllegalArgumentException(String.format("You passed an invalid level: %d.", level));
        }
    }
