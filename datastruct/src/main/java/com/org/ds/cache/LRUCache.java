...
/**
 * A bounded com.org.ds.cache that evicts keys based on least recently used strategy.
 * 
 * @param <CacheKeyType>
...
public class LRUCache<CacheKeyType, CacheValueType> implements Cache<CacheKeyType, CacheValueType> {
    ...
    private void checkCapacity(int capacity) {
        if (capacity <= 0) {
            throw new IllegalArgumentException(String.format("Capacity should be positive. You passed: %d", capacity));
        }
    }
    ...
    private CacheNode<CacheKeyType, CacheValueType> evict() {
        if (head == null) {
            throw new AssertionError("Cannot evict a node from an empty com.org.ds.cache.");
        }
    ...
    private static class CacheNode<CacheNodeKeyType, CacheNodeValueType> {
        ...
        public void setPrevious(CacheNode<CacheNodeKeyType, CacheNodeValueType> previous) {
