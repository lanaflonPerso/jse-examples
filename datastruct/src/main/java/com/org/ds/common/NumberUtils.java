...
public class NumberUtils {
    ...
    public static int atoi(String s) {
        ...
        if (s.charAt(0) == '+' || s.charAt(0) == '-') {
            ...
            if (s.length() == 1) {
                throw new NumberFormatException(String.format("The input string: %s is not a valid integer.", s));
            }
        }
        ...
        for (int index = startIndex; index < s.length(); index++) {
            ...
            num = num * 10 + (c - '0');
            if (num > Integer.MAX_VALUE && !(isNegative && num == Integer.MAX_VALUE + 1)) {
                throw new NumberFormatException(String
                        .format("The input string: %s is not a valid integer as it overflows the integer range.", s));
            }
        }
