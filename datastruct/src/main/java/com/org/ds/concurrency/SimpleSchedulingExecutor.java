...
public class SimpleSchedulingExecutor implements SchedulingExecutor {
    ...
    public SimpleSchedulingExecutor(int threadPoolSize) {
        ...
        Comparator<Pair<Runnable, Long>> comparator = new Comparator<Pair<Runnable, Long>>() {
            @Override
            public int compare(Pair<Runnable, Long> foo, Pair<Runnable, Long> bar) {
    ...
    public static void main(String...argsargs) throws InterruptedException {
        ...
        Runnable secondTask = new Runnable() {
            @Override
            public void run() {
                System.out.printf("Performing task: %d at %d\n", secondTaskId, System.currentTimeMillis());
            }
        };
        ...
        Runnable thirdTask = new Runnable() {
            @Override
            public void run() {
                System.out.printf("Performing task: %d at: %d\n", thirdTaskId, System.currentTimeMillis());
            }
        };
        ...
        Runnable fourthTask = new Runnable() {
            @Override
            public void run() {
                System.out.printf("Performing task: %d at: %d\n", fourthTaskId, System.currentTimeMillis());
            }
        };
        ...
        Runnable fifthTask = new Runnable() {
            @Override
            public void run() {
                System.out.printf("Performing task: %d at: %d\n", fifthTaskId, System.currentTimeMillis());
            }
        };
    ...
    public void schedule(Runnable task, long epochTime) {
        ...
        if (epochTime < currentTime) {
            new IllegalArgumentException(
                    String.format("Can't execute a task with time: %d which starts before current system time: %d.\n",
        ...
        try {
            ...
            if (runnables.containsKey(task)) {
                throw new IllegalArgumentException(
                        String.format("Task: %s has already been scheduled to run at: %d.", task, runnables.get(task)));
            }
    ...
    public void delete(Runnable task) {
        ...
        try {
            ...
            if (!runnables.containsKey(task)) {
                throw new IllegalArgumentException(
                        String.format("Task: %s was never scheduled earlier, or is running now.", task));
            }
    ...
    private class Dispatcher implements Runnable {
        ...
        private void dispatch() {
            try {
                ...
                System.out.printf("Caught RuntimeException in dispatcher loop: %s\n", e);
            } finally {
                lock.unlock();
            }
