...
public class CountBinarySubsequences {
    ...
    public static void main(String...argsargs) {
        ...
        System.out.println("\n================\n");
        System.out.println("\n\nAnswer: " + obj.countSubSequences("11001", "101"));
        System.out.println("\n================\n");
        System.out.println("\n\nAnswer: " + obj.countSubSequences("1001101011", "1011"));
        System.out.println("\n================\n");
    }

    /**
     * Returns the number of times the binary string "needle" occurs as a subsequence in the binary string "haystack".
     * The solution below uses dynamic programming!
     *
     * Recursive definition of subsequence counting:
     *
     * We have the two com.org.ds.strings: Needle[1 ... N] and Haystack [1 ... M]. We need to find # of occurences of
     * needle in haystack. Lets call Needle as X and Haystack as H.
     *
     * SubsequenceCount(X[P ... N], H[Q ... M]) =
     *
     * if X[P] != H[Q]: SubsequenceCount(X[P ... N], H[Q+1 ... M]) otherwise: SubsequenceCount(X[P+1 ... N], H[Q+1 ...
     * M]) + SubsequenceCount(X[P ... N], H[Q+1 ... M])
     *
     * @param haystack
    ...
    public long countSubSequences(String haystack, String needle) {
        ...
        String haystackReverse = new StringBuffer(haystack).reverse().toString();
        ...
        for (int row = 1; row < dpTable.length; row++) {
            for (int col = 1; col < dpTable[0].length; col++) {
                if (needleReverse.charAt(row - 1) == haystackReverse.charAt(col - 1)) {
                    dpTable[row][col] = dpTable[row - 1][col - 1] + dpTable[row][col - 1];
                } else {
    ...
    private void printDpTable(long[][] dpTable, String needle, String haystack) {
        ...
        haystack = new StringBuffer(haystack).reverse().insert(0, "X").toString();
