...
/**
 * Given two text com.org.ds.strings A of length n and B of length m, you want to transform A into B with a minimum
 ...
 *
 * Write a function to compute the edit distance between two given com.org.ds.strings.
 */
    ...
    public static int findEditDistance(String foo, String bar) {
        ...
        for (int fooIndex = 1; fooIndex <= foo.length(); fooIndex++) {
            for (int barIndex = 1; barIndex <= bar.length(); barIndex++) {
                int deleteAndTransform = dpTable[fooIndex - 1][barIndex] + COST_OF_DELETION;
                int insertAndTransform = dpTable[fooIndex][barIndex - 1] + COST_OF_INSERTION;
                ...
                dpTable[fooIndex][barIndex] = min(deleteAndTransform, insertAndTransform, replaceAndTransform);
            }
        }
