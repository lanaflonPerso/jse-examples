...
public class LexicographicalRanking {
    public static int findLexicographicalRank(String s) {
        ...
        if (s.length() == 0) {
            throw new IllegalArgumentException("Can't find rank for empty string.");
        }

        int rank = 0;
        int exponent = s.length() - 1;
        for (int index = 0; index < s.length(); index++) {
            rank += new Double(Math.pow(26, exponent)).intValue() * (getRank(s.charAt(index)));
            exponent--;
        }
    ...
    private static int getRank(char c) {
        if (c < 'a' || c > 'z') {
            throw new IllegalArgumentException(String.format("Character is not in range: %c to %c", 'a', 'z'));
        }
