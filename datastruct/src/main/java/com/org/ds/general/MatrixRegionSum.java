...
/**
 * Given a matrix of integers and coordinates of a rectangular region within the matrix, find the sum of numbers falling
 * inside the rectangle. The program will be called multiple times with different rectangular regions from the same
 * matrix.
 */
    ...
    public long findSum(int lx, int ly, int rx, int ry) {
        if (!valid(lx, ly) || !valid(rx, ry)) {
            throw new IllegalArgumentException(
                    String.format("The co-ordinates: (%d, %d), (%d, %d) are not valid co-ordinates.", lx, ly, rx, ry));
        }
