...
/**
 * You are given a maze (2D boolean matrix) where true = free way, false = obstacle. Write a function to find out how
 * many ways exist to reach [N][M] from [0][0].
 */
public class Maze {
    ...
    private static long findNumWays(boolean[][] maze, int row, int col, boolean[][] visited) {
    ...
    private static boolean endOfMaze(boolean[][] maze, int row, int col) {
        return maze != null && row == maze.length - 1 && col == maze[0].length - 1;
    }

    private static boolean isValid(boolean[][] maze, int row, int col) {
        return maze != null && row >= 0 && row < maze.length && col >= 0 && col < maze[0].length;
    }
}
