...
/**
 * You are given a series of non-intersecting intervals sorted in non-decreasing order of their starting points. Write a
 * function which when given another interval, merges it into these existing intervals.
 *
 * Examples:
 *
 * [1] merge([12,13], { [1,2], [4,10], [17,22] }) => {[1,2], [4,10], [12,13], [17,22]}
 *
...
public class MergeIntervals {
    ...
    public static List<Interval> mergeIntervals(List<Interval> intervals, Interval newInterval) {
        if (intervals == null) {
            throw new NullPointerException("The list of intervals passed is null.");
        }
    ...
    private static List<Interval> doMergeIntervals(List<Interval> intervals, Interval newInterval) {
        ...
        while (iter.hasNext()) {
            ...
            if (isIntersecting(current, newInterval)) {
                ...
                Interval combinedInterval = new Interval(Math.min(current.getFirst(), newInterval.getFirst()),
                        Math.max(last.getSecond(), newInterval.getSecond()));
