...
/**
 * Given a string 'S', generate the next lexicographical permutation of the same size as string 'S'. If no such
 * permutation exists, return null. Assume that each permutation (including the input) has unique characters, and the
 * character set is lowercase english alphabets ('a' to 'z').
 *
 * http://pythonadventures.wordpress.com/2012/03/21/lexicographically-next- permutation/
 */
public class NextLexciographicalPermutation {
    ...
    public static String nextLexciographicalPermutation(String s) {
        ...
        while (swapIndex >= 0 && getRank(array[swapIndex]) > getRank(array[swapIndex + 1])) {
            swapIndex--;
        }
