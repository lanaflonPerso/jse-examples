    ...
    public static void printCombinations(char[] array, int count) {
        ...
        if (count <= 0 || count > array.length) {
            throw new IllegalArgumentException(String.format(
                    "count should be in the range: %d to %d (inclusive). You passed: %d", 1, array.length, count));
        }
        printCombinations(array, 0, count, new char[count], 0);
    }

    private static void printCombinations(char[] array, int arrayIndex, int count, char[] combination,
        ...
        for (int index = arrayIndex; index < array.length; index++) {
            ...
            printCombinations(array, index + 1, count - 1, combination, combinationIndex + 1);
        }
    }
}
