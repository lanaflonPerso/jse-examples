...
public class PowerFunctions {
    ...
    private static long pow(long x, long y) {
        if (y < 0) {
            throw new IllegalArgumentException(String.format(" y can't be negative. you passed: %d", y));
        }
    ...
    public static int squareRoot(int number) {
        if (number < 0) {
            throw new IllegalArgumentException(String.format("Number cannot be negative. You passed: %d", number));
        }
        ...
        while (start <= end) {
            ...
            System.out.println(String.format("start: %f, end: %f, middle: %f\n", start, end, middle));
