 ...
 * formed friendships, design an algorithm to determine the earliest time at which all members are connected (i.e.,
 * every member is a friend of a friend of a friend ... of a friend).
 * 
 * Assume that the log file is sorted by timestamp and that friendship is an equivalence relation (i.e. it is reflexive,
 * symmetric and transitive). The running time of your algorithm should be M logN or better and use extra space
 * proportional to N.
 * 
 ...
 * 
 * "T1: A B" => This means A and B became friends at time T1. Note that T1 < T2 < T3 etc. (i.e. file is sorted based on
...
class WeightedUnionFind {
    ...
    public WeightedUnionFind(int size) {
        if (size < 0) {
            throw new IllegalArgumentException(
                    String.format("Size of the graph has to be non-negative. You passed: %d", size));
        }
        this.size = size;
        graph = new int[size];
        numRoots = size;
    ...
    private void checkNodeIndex(int index) {
        if (index < 0 || index >= size) {
            throw new IllegalArgumentException(
                    String.format("Index of a node in the graph has to be >= 0 and < %d. You passed: %d", size, index));
        }
    }
