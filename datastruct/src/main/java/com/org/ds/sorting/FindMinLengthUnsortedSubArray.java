...
/**
 * Find the minimum length unsorted subarray, which when sorted makes the array completely sorted.
 */
public class FindMinLengthUnsortedSubArray {

    public static Pair<Integer, Integer> findMinLengthUnsortedSubArray(int[] array, Comparator<Integer> comparator) {
        if (array == null || array.length == 1) {
            return null;
        }

        // Find first index where com.org.ds.sorting order is disturbed.
        int startIndex;
    ...
    public static void main(String...argsargs) {
        ...
        solution = findMinLengthUnsortedSubArray(array, new Comparator<Integer>() {
            @Override
            public int compare(Integer a, Integer b) {
                return a - b;
            }
