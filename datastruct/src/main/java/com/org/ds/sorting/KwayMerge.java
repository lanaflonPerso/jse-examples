...
/**
 * Merge K sorted com.org.ds.arrays into a single array of size N using a heap. Complexity is O(log K) for
 * heapification, and O(K) to build the heap. We have N total elements, and therefore after building the heap initially
 * we need to heapify N times, each time popping the element from the top of the heap. Therefore, the total time
 * complexity is O(K) + O(N log K) = O(N log K).
 */
public class KwayMerge {
    public static <T> T[] merge(Class<T> type, T[][] input, final Comparator<T> comparator) {
        if (input == null) {
        ...
        PriorityQueue<Pair<Integer, T[]>> heap = new PriorityQueue<Pair<Integer, T[]>>(input.length, pairComparator);
    ...
    private static <T> int totalLength(T[][] input) {
        ...
        for (T[] array : input) {
            if (array == null) {
                throw new NullPointerException("nulls are not allowed in the input array.");
            }
    ...
    public static void main(String...argsargs) {
        ...
        System.out.println(Arrays.toString(merge(Integer.class, a, comparator)));
    }
}
