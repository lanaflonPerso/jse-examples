...
/**
 * Merge two sorted com.org.ds.arrays A of length M and B of length N into the same array A. Assume that A has enough
 * space in the end to accomodate all elements of B. i.e. A's size is M + N.
 */
public class MergeWithSpaceForOtherArray {
    public static <T> void merge(T[] bigArray, int bigArraySize, T[] smallArray, Comparator<T> comparator) {
        ...
        while (smallArrayIndex >= 0 && bigArrayIndex >= 0) {
            if (comparator.compare(smallArray[smallArrayIndex], bigArray[bigArrayIndex]) < 0) {
