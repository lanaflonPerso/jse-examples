...
public class QuickSort {
    ...
    public static <T> int partition(T[] array, int startIndex, int endIndex, Comparator<T> comparator, int pivotIndex) {
        ...
        com.org.ds.common.ArrayUtils.swap(array, pivotIndex, endIndex);
        int swapIndex = startIndex - 1;
        for (int index = startIndex; index <= endIndex - 1; index++) {
            if (comparator.compare(array[index], array[endIndex]) <= 0) {
                swapIndex++;
                com.org.ds.common.ArrayUtils.swap(array, swapIndex, index);
            }
        }

        com.org.ds.common.ArrayUtils.swap(array, swapIndex + 1, endIndex);
        return swapIndex + 1;
    }
}
