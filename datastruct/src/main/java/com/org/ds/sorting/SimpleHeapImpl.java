...
public class SimpleHeapImpl<T> implements Heap<T> {
    ...
    public T pop() throws EmptyHeapException {
        ...
        com.org.ds.common.ListUtils.swap(heap, 0, heap.size() - 1);
    ...
    private void heapify(int length, int rootIndex) {
        ...
        while (true) {
            if (leftChildIndex(rootIndex) < length) {
                if (comparator.compare(heap.get(leftChildIndex(rootIndex)), heap.get(indexToBeSwapped)) > 0) {
                    indexToBeSwapped = leftChildIndex(rootIndex);
                }
            }

            if (rightChildIndex(rootIndex) < length) {
                if (comparator.compare(heap.get(rightChildIndex(rootIndex)), heap.get(indexToBeSwapped)) > 0) {
            ...
            com.org.ds.common.ListUtils.swap(heap, rootIndex, indexToBeSwapped);
