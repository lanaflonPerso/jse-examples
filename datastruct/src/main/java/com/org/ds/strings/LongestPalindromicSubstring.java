...
public class LongestPalindromicSubstring {
    public static String getLongestPalindromicSubstring(String s) {
        ...
        return s.substring(maxPalindrome.getFirst(), maxPalindrome.getSecond() + 1);
    }

    private static void getLongestPalindromicSubstring(String s, Pair<Integer, Integer> current,
    ...
    private static boolean exceeded(Pair<Integer, Integer> current, Pair<Integer, Integer> maxPalindrome) {
        int startIndex = current.getFirst(), endIndex = current.getSecond();
        int maxStartIndex = maxPalindrome.getFirst(), maxEndIndex = maxPalindrome.getSecond();

        if (maxStartIndex == -1 || (endIndex - startIndex + 1) > (maxEndIndex - maxStartIndex + 1)) {
            return true;
        }
