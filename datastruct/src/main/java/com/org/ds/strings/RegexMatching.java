 ...
 * isMatch("aa", ".*") -> true isMatch("ab", ".*") -> true isMatch("aab", "c*a*b") -> false
 */
public class RegexMatching {
    ...
    private static boolean isMatch(String str, int strIndex, String regex, int regexIndex) {
        ...
        if (regexIndex < regex.length() - 1) {
            ...
            if (nextRegexChar == '*') {
                ...
                if (regexChar == '.') {
                    for (int wildCardMatcher = strIndex; wildCardMatcher < str.length(); wildCardMatcher++) {
                ...
                for (int wildCardMatcher = strIndex; wildCardMatcher < str.length()
                        && str.charAt(wildCardMatcher) == regexChar; wildCardMatcher++) {
                    if (isMatch(str, wildCardMatcher + 1, regex, regexIndex + 2)) {
                        return true;
                    }
                }
