    ...
    private static void reverse(char[] array, int startIndex, int endIndex) {
        ...
        if (startIndex > endIndex) {
            throw new IllegalArgumentException(
                    String.format("invalid startIndex: %d and endIndex: %d", startIndex, endIndex));
        }

        if (startIndex < 0) {
            throw new IllegalArgumentException(String.format("invalid startIndex: %d", startIndex));
        }

        if (endIndex >= array.length) {
            throw new IllegalArgumentException(String.format("invalid endIndex: %d", endIndex));
        }
