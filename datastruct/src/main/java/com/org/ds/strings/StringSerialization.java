 ...
 *
 * i.e. Assume: String...argscom.org.ds.strings = {"abcd", "efgh"};
 *
 * Then, the following always returns true as the serialize and deserialize methods are consistent:
 *
 * java.util.Arrays.equals(deserializeStrings(serializeStrings(com.org.ds.strings)), com.org.ds.strings);
 */
public class StringSerialization {
    /**
     * Serializes an array of com.org.ds.strings into bytes.
     *
     * @param strings
     *            array of com.org.ds.strings to be deserialized
     * @return array of bytes representing the string
     */
    public static byte[] serializeStrings(String[] strings) {
        if (strings == null) {
            throw new NullPointerException("com.org.ds.strings can't be null.");
        }
    ...
    /**
     * Deserializes an array of com.org.ds.strings from bytes. NOTE: The array of com.org.ds.strings should have been
     * serialized using: {@link #serialize(String[])}.
     *
     * @param bytes
     *            array of bytes to be deserialized
     * @return deserialized array of com.org.ds.strings
     */
    public static String[] deserializeStrings(byte[] serialized) {
        ...
        for (int count = 0; count < length; count++) {
            ...
            if (strLength == -1) {
                ...
                strings[count] = deserializeString(serialized, nextByte, strLength);
    ...
    public static void serializeInt(byte[] serialized, int offset, int number) {
        if (offset > serialized.length) {
            throw new AssertionError(
                    String.format("offset: %d can't be greater than length of array: %d", offset, serialized.length));
        }
    ...
    public static String deserializeString(byte[] serialized, int offset, int length) {
