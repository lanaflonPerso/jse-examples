...
/**
 * Write functions to serialize/deserialize an N-ary tree containing only com.org.ds.strings to/from a byte stream.
 *
 * The serialization format is the following:
 *
 * |NUMBER OF NODES| NODE METADATA | STRINGS | (int32) (int32, int32)[] string[]
 */
public class NaryTreeSerialization {
    ...
    private static void serializeMetadata(BufferedOutputStream fileHandle, List<Pair<Integer, Integer>> metadata)
            throws IOException {
    ...
    private static List<Pair<Integer, Integer>> deserializeMetadata(BufferedInputStream fileHandle, int metadataSize)
            throws IOException {
    ...
    private static String deserializeString(BufferedInputStream fileHandle, int length) throws IOException {
        if (length < 0) {
            throw new IllegalArgumentException(
                    String.format("Length should be greater than zero. You passed: %d.", length));
        }
