...
public class ServiceLocator {
    ...
    /**
     * Fetch the service with the name param from the cache first, if no service is found, lookup the service from the
     * {@link InitContext} and then add the newly created service into the cache map for future requests.
     *
    ...
    public static Service getService(String serviceJndiName) {
        ...
        if (serviceObj != null) {
            ...
            /*
             * If we are unable to retrive anything from cache, then lookup the service and add it in the cache map
             */
